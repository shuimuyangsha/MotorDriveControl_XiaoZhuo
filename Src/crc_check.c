...
/******************************************************************************
 * Function:    uint16_t CRC16 (uint8_t *data, uint16_t len )
 * Description: CRC校验码计算函数,输入数据为8位
 * Input:       data:指向将要进行发送数据的首地址
 *				len:数据长度
 * Output:      no
 * Return:      no
 * Others:
...
uint16_t crc16(uint8_t *data, uint16_t len )
{
    ...
    while (len--)
    {
        i = uchCRCLo ^ (*data++); //当有效数据小于请求数据时无效数据为随机值，高8为不为0导致CRC计算出错
        uchCRCLo = uchCRCHi ^ (auchCRC[i] >> 8);
        uchCRCHi = auchCRC[i];
    }
    ...
    while (len--)
    {
        i = uchCRCLo ^ (*data++ & 0x00FF); //当有效数据小于请求数据时无效数据为随机值，高8为不为0导致CRC计算出错
